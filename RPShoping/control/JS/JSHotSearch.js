jQuery(document).ready(function () {
    ...
    function getproductsort(callback) {
        ...
        $.ajax({
            url: "https://d.apicloud.com/mcm/api/tb_HotSearch?filter=" + encodeURIComponent(JSON.stringify(filter)),
            method: "get",
            headers: {
    ...
    $(document.body).on("click", "#add", function () {
        window.location.href = "HotSearchEdit.aspx?m_id=" + _m_id;
    });
    //刷新
    ...
    $(document.body).on("click", "#edit", function () {
        ...
        if (ids.length <= 0 || ids.length > 1) {
            ...
            window.location.href = "HotSearchEdit.aspx?id=" + temprowData.id + "&m_id=" + _m_id;
        }
    });
        ...
        if (ids.length <= 0) {
            ...
            AlertConfirm("您确定要删除选中的数据吗？", function (c) {
                if (c == "1") {
                    ...
                    $.ajax({
                        url: "https://d.apicloud.com/mcm/api/tb_HotSearch/" + id,
                        method: "post",
                        headers: {
                            //"Content-type": "application/json",
                            "X-APICloud-AppId": _appid,

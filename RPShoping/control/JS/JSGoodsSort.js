        ...
        getproductsort(function (c, data) {
            if (c == "1") {
                ...
                $("#list5").jqGrid({
                    datatype: "local",
                    data: t,
                    colNames: ['序号', '分类名称','图标名称', '上级id','是否删除', '备注'],
                    colModel: [
                        { name: 'id', index: 'id', width: 55, align: "center", hidden: true },
                        { name: 'PS_Name', index: 'PS_Name', width: 80, align: "center" },
                        { name: 'PS_icon', index: 'PS_icon', width: 80, align: "center" },
                        { name: 'PS_ParentID', index: 'PS_ParentID', width: 155, align: "center", hidden: true },
                        { name: 'isdelete', index: 'isdelete', width: 80, align: "center" },
                        { name: 'PS_Remark', index: 'PS_Remark', width: 120, align: "center" }
                    ],
                    page: 1,
                    rowNum: 30,
                    rowList: [30, 50, 100],
...
//获取商品分类信息
    function getproductsort(callback) {
        ...
        $.ajax({
            url: "https://d.apicloud.com/mcm/api/tb_ProductSort?filter=" + encodeURIComponent(JSON.stringify(filter)),
            method: "get",
            headers: {
    ...
    $(document.body).on("click", "#add", function () {
        window.location.href = "GoodsSortEdit.aspx?m_id=" + _m_id;
    });
    //刷新
    ...
    $(document.body).on("click", "#edit", function () {
        ...
        if (ids.length <= 0 || ids.length > 1) {
            ...
            window.location.href = "GoodsSortEdit.aspx?id=" + temprowData.id + "&m_id=" + _m_id;
        }
    });

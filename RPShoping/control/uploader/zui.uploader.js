...
(function($, window, Plupload, Moxie, undefined) {
    ...
    // removeAllFiles(include static-files) --add by bukaa
    Uploader.prototype.removeAllFiles = function() {
    	var that = this;
    	that.$.find(".file").remove();
		var oldFiles = that.getFiles();
		$.each(oldFiles, function(idx, file) {
			that.removeFile(file.id);
        });
		that.showStatus();
    };
    
    //show static-files from server --add by bukaa
    Uploader.prototype.showAllFiles = function(url, downloadUrlPre) {
    	if(!url)return;
    	var that = this;
    	that.removeAllFiles();
    	$.get(url, function(data){
			if(data != null && data.length > 0){
				$.each(data, function(idx, file) {
	                file = $.extend({status: $.zui.plupload.DONE, remoteData: {"key":file.fkey}}, file);
	                if(!downloadUrlPre){
	                	file.url = "../file/downloadCommonFile?key="+file.fkey;
	                }else{
	                	file.url =  downloadUrlPre+"?key="+file.fkey;
	                }
	                file.static = true;
	                that.showFile(file);
	            });
